...
void CommandHandler::handleTopic(Client& client, const Message& message, std::vector<Client*>& annular) {
    ...
    if (newTopic.empty()) {
        if (channel->topic.empty()) {
            client.appendWriteBuffer(":ircserv 331 " + client.getNickname() + " " + channelName + " :No topic is set");
        } else {
            client.appendWriteBuffer(":ircserv 332 " + client.getNickname() + " " + channelName + " :" + channel->topic);
        }
        return;
    }
    ...
    for (std::map<int, Client*>::iterator mit = members.begin(); mit != members.end(); ++mit) {
        mit->second->appendWriteBuffer(topicMsg);
    }
}
